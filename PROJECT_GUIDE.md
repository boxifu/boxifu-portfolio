...
- `embeds` adds reusable project media or tools after the Markdown body. Supported types are `youtube`, `googleSlides`, `googleDocs`, `googleSheets`, `localDocument`, `itch`, `steam`, and `figma`.
- Use `group: "documents"` for design docs, slide decks, spreadsheets, PDFs, and other development artifacts; `group: "video"` for YouTube videos; and `group: "play"` for playable/store embeds such as itch.io or Steam widgets. The project template renders document previews first, video embeds centered below them, and playable/store embeds last. Document previews wrap into centered rows, so one document or an odd final row should be centered automatically.
- Use live Google embeds first when they actually render in the page: `googleDocs`, `googleSheets`, or `googleSlides` with a normal shared/edit URL. The renderer converts these to `/preview` iframe URLs.
- Google sharing and Google iframe embedding are not equivalent. If a Google iframe shows "access denied", "document is not published", or a blank/auth screen even though the Open button works, switch that item to `localDocument`.
- Use `localDocument` when Google iframe preview is unreliable or when the file should live in the repository. Store the static preview HTML in `public/assets/previews/projects/project-slug/`. `previewSrc` points to that HTML file, and `fileSrc` should still point to the best full-resolution source, such as the live Google Slides sharing link or a local PDF.
- For Google Slides specifically, "Anyone with the link can view" may still fail inside an iframe. A good fallback is a local title-slide or exported-slide preview plus an Open button to the live deck.
- Steam widgets use `type: "steam"`, `group: "play"`, and the widget URL, for example `https://store.steampowered.com/widget/2858380/`.
- Figma embeds use `type: "figma"`, `group: "documents"`, and the Figma embed URL in `url`, usually starting with `https://embed.figma.com/design/...`. Set `fileSrc` to the normal shared Figma file URL so the Open button gives viewers a full-page fallback.
